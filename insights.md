...
This guidebook distils design decisions and empirical findings from the current note inventory of **89 biology foundation-model (bio-FM) papers** that meet a strict definition: pretrained on biological data at scale, learning a general-purpose representation transferable to multiple downstream tasks. **Rev 4** added 15 newly-extracted FMs (GET, Evo 2, ESM-3, AlphaFold 3, RoseTTAFold All-Atom, ProteinMPNN, Nicheformer, UCE, CellPLM, GenePT, SCimilarity, scMulan, AIDO, Virchow2, CONCH-NatMed); the 2026 practitioner refresh re-weights the practitioner-facing guidance using the now-current DNA/RNA/multimodal evidence base, including JEPA-DNA, Orthrus, MIMIC, scELMo, and scMamba.

**Rev 5** re-read every one of the then-current 86 FM notes in five parallel batches with Opus-4.7 extraction agents, collecting TL;DR, Model, Data, Training Recipe, Ablations Rev 4, Key Ablations, and Reported Insights sections. TranscriptFormer, PerturbDiff, and Decima have since been added to the note inventory. The resulting per-axis evidence index is consolidated under `.cache/rev5_*` and used to refresh the Cheatsheet/Catalogue counts and surface previously-missed numeric ablations. The remaining 57 surveyed papers are supporting/baseline non-FMs (e.g., TAPE, scVI, totalVI, Cellpose, CLAM); these may appear as benchmarks but are **never** counted in the per-claim `(N=X papers)` evidence tallies that follow.
...
| Gene expression from DNA | [Enformer](https://doi.org/10.1038/s41592-021-01252-x) ([note](notes/effective-gene-expression-prediction-2021.md)) / [Borzoi](https://doi.org/10.1038/s41588-024-02053-6) ([note](notes/predicting-rna-seq-coverage-2023.md)); [Decima](https://doi.org/10.1038/s41592-026-03102-0) ([note](notes/decoding-sequence-determinants-of-2026.md)) for cell/disease state outputs | CNN+Transformer | Supervised on tracks or sc/snRNA-seq targets | 196kb→524kb gives small Pearson lift; Decima adds cellular-context targets |
...
| [Decima](https://doi.org/10.1038/s41592-026-03102-0) ([note](notes/decoding-sequence-determinants-of-2026.md)) | Training a Borzoi-initialized sequence-to-expression model on >22M sc/snRNA-seq profiles shifts regulatory prediction from bulk tracks toward cell-type- and disease-state-specific outputs; accessible sources do not yet expose numeric ablations, so this is qualitative-only evidence. |
...
| [Decima](https://doi.org/10.1038/s41592-026-03102-0) ([note](notes/decoding-sequence-determinants-of-2026.md)) | Decima uses sc/snRNA-seq-derived cellular contexts as prediction targets for a Borzoi-initialized DNA model, making cell type and disease state part of the sequence-to-function output rather than a post-hoc annotation layer. |
...
[Enformer](https://doi.org/10.1038/s41592-021-01252-x) ([note](notes/effective-gene-expression-prediction-2021.md)) (Enformer, 196kb) and [Borzoi](https://doi.org/10.1038/s41588-024-02053-6) ([note](notes/predicting-rna-seq-coverage-2023.md)) (Borzoi, 524kb) remain the two-model recipe for aggregate tracks. [Decima](https://doi.org/10.1038/s41592-026-03102-0) ([note](notes/decoding-sequence-determinants-of-2026.md)) should be tracked when the target is cell type- or disease-state-specific expression from sequence. [GET](https://doi.org/10.1038/s41586-024-08391-z) ([note](notes/a-foundation-model-of-2025.md)) (GET) extends to cell-type-conditioned cross-tissue prediction with accessibility/motif inputs; [MIRROR-3D](https://arxiv.org/abs/2504.09060) ([note](notes/multimodal-3d-genome-pre-2025.md)) (MIRROR-3D) adds Hi-C fusion. **(N=5 papers)** epigenome FMs: [Enformer](https://doi.org/10.1038/s41592-021-01252-x) ([note](notes/effective-gene-expression-prediction-2021.md)), [Borzoi](https://doi.org/10.1038/s41588-024-02053-6) ([note](notes/predicting-rna-seq-coverage-2023.md)), [Decima](https://doi.org/10.1038/s41592-026-03102-0) ([note](notes/decoding-sequence-determinants-of-2026.md)), [GET](https://doi.org/10.1038/s41586-024-08391-z) ([note](notes/a-foundation-model-of-2025.md)), [MIRROR-3D](https://arxiv.org/abs/2504.09060) ([note](notes/multimodal-3d-genome-pre-2025.md))
...
[Decima](https://doi.org/10.1038/s41592-026-03102-0) ([note](notes/decoding-sequence-determinants-of-2026.md)) is a bridge case rather than a replacement for single-cell representation FMs: it uses large sc/snRNA-seq atlases as targets for DNA sequence-to-expression prediction, so use it for cell-type-resolved regulatory/variant questions, not for embedding cells from expression profiles.
